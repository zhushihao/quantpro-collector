...
Market-ledger 原生 Collector 工具优先；若 ChatGPT Connector 快照尚未暴露新增工具，只允许通过 QuantPro RESEARCH 固定仓库中的 `scripts/market-ledger-cli.mjs` compatibility bridge 读写同一 Issue #2。禁止任意 shell、直接 `gh`、任意 repo/issue/token 参数或 Research Job 写入。

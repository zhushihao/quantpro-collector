...
3. Collector 提供业务数据、LIVE facts、Research replica / Research Job 协议，以及 holding-assistant 的结构化 market-ledger 读写接口；不得承载或分发 Automation Prompt。
4. Web 只用于业务 Prompt 明确要求的最新外部事实扫描；不得承担 Prompt/Guidance 控制面或 market-ledger 运行态读取。
5. holding-assistant 的 GitHub Issue #2 只作为审计落点；Scheduled Task 不直接分页或写 GitHub，也不依赖 GitHub Plugin / Connector。运行态统一通过 Collector `get_market_checkpoints` / `append_market_checkpoint`。
...
`WRITE_SCOPE` 约束 Collector / Research MCP 的业务写权限。`MARKET_LEDGER_APPEND_ONLY` 只允许 Collector 的固定 Issue #2 append 接口，不授权 Research Job 写入、通用 GitHub 写入或其他目标。
